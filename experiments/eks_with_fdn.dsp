declare name "EKS Guitar Sequencer with FDN Reverb";
...
// BPM for 16th notes: multiply by 4 to get 16th note rate
bpm_base = hslider("tempo [unit:BPM]", 180, 60, 240, 1);
bpm = bpm_base * 4; // Convert to 16th note rate

// Beat counter
beat_seq = ba.beat(bpm) : ba.pulse_countup_loop(31, 1);

// Step counter (0-31)
step = beat_seq : int;

// Detect transitions to trigger notes
trigger = (beat_seq > beat_seq') * run;
...
// Gate is the transition trigger
gate = trigger;
...
noiseburst(g,P) = no.noise : *(g : trigger_gen(P))
with {
  diffgtz(x) = (x-x') > 0;
  decay(n,x) = x - (x>0)/n;
  release(n) = + ~ decay(n);
  trigger_gen(n) = diffgtz : release(n) : > (0.0);
};
...
reverb_size = hslider("reverb_size", 0.7, 0.1, 1.0, 0.01);
reverb_fb = hslider("reverb_feedback", 0.7, 0, 0.95, 0.01);

// Simple stereo reverb using re.mono_freeverb
simple_reverb = re.mono_freeverb(reverb_fb, reverb_size, 5000, 0);

// Stereo effect
reverb_effect(dry_l, dry_r) = out_l, out_r
with {
    mono = (dry_l + dry_r) / 2;
    wet = mono : simple_reverb;
    out_l = dry_l*(1-reverb_mix) + wet*reverb_mix;
    out_r = dry_r*(1-reverb_mix) + wet*reverb_mix;
};

process = filtered_excitation : stringloop
          <: _, widthdelay : stereopanner(A) : reverb_effect;
